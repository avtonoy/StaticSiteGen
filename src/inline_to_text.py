    ...
    for node in old_node:
        ...
        if node.text_type == TextType.TEXT:
            ...
            for element in elements:
                ...
                if len(res[0]) > 0:
                    new_nodes.append(
                        TextNode(text=res[0], text_type=TextType.TEXT))
                new_nodes.append(
                    TextNode(text=element[0], text_type=url_type, url=element[1]))
                text = res[1]
            if len(text) > 0:
...
def text_to_textnodes(text: str) -> list[TextNode]:
    ...
    italic_node = split_nodes_delimiter(
        bold_node, '_', text_type=TextType.ITALIC)
    code_node = split_nodes_delimiter(
        italic_node, '`', text_type=TextType.CODE)
